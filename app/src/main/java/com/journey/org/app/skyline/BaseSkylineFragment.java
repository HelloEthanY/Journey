 ...
 * @Date 2019/3/20
 */
public abstract class BaseSkylineFragment<V extends ViewDataBinding, VM extends BaseViewModel> extends BaseFragment<V, VM> {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        AndroidBarUtil.statusBarHide(getActivity());
        statusBarHeight = AndroidBarUtil.getStatusBarHeight(getActivity());
        // 给另起动的进程设置当前进程所在的Activity
        TEApp.setMainActivityContext(mActivity);
        // 新进程链接初始化
        AppLinks.initializeAsync();
        // 初始化核心服务
        CoreServices.Init(mActivity);
    }
    ...
    private void requestPermissions() {
        RxPermissions mRxPermissions = new RxPermissions(Objects.requireNonNull(getActivity()));
    ...
    private void showPermissionsDialog() {
        MaterialDialogUtils.showBasicDialog(getActivity(), "系统提示", "需要文件的读写权限才能进行加载三维地图，请赋予文件的读写权限！")
